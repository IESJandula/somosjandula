...
watch(selectedCommands, (newValue) => {
...
const isCommandSelected = (command) => {
    return selectedCommands.value.some(c => c.modelName + c.action === command.modelName + command.action);
};

// Function to handle action selection
const toggleCommandSelection = (command) => {
    const key = command.modelName + command.action;
    const selectedIndex = selectedCommands.value.findIndex(c => c.modelName + c.action === key);
...
watch(
    [currentPage, selectedCommandModel, selectedCommandAction],
    () => {
        const commandModelFilter = selectedCommandModel.value !== "default" ? selectedCommandModel.value : null;
        const commandActionFilter = selectedCommandAction.value !== "default" ? selectedCommandAction.value : null;

        emit('pageUpdate', currentPage.value, 5, commandModelFilter, commandActionFilter);
    }
);
...
<template>
    <div>
        ...
        <div class="col pb-3 mx-auto">
            <ComboBoxModel v-model="selectedCommandModel" :labelValue="'Modelo:'" :key="selectedCommandModel"
        ...
        <table class="table border table-bordered border-dark text-center m-0">
            ...
            <tbody>
                <tr v-for="(command) in commandsList" :key="command.modelName+command.action" @click="toggleCommandSelection(command)"
                    class="clickable-row" :class="{ 'table-danger': isCommandSelected(command) }">
                    <td>
                        {{ command.modelName }}
                    </td>
                ...
                <tr v-if="commandsList.length === 0">
                    <td colspan="3"><strong>No se han encontrado registros de comando.</strong></td>
                </tr>
        ...
        <div class="d-flex justify-content-evenly p-2">
            <button class="btn btn-warning" @click="discardSelection"><i class="bi bi-file-earmark-fill"></i>
                Descartar</button>
            <button class="btn btn-danger" @click="emitDeletionRequest">
