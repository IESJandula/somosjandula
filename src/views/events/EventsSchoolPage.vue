<template>
    <div class="year-calendar">
        ...
        <div class="months-grid">
            <template v-for="(month, index) in months" :key="month.name">
                ...
                <div class="month-card">
                    ...
                    <div class="days-grid">
                        <span v-for="n in getFirstDayOffset(month.month, month.year)" :key="'e-' + n" class="empty-day">
                        </span>

                        <div v-for="day in getDaysInMonth(month.month, month.year)" :key="day" class="day-cell"
                            @click="onDayClick(month.month, month.year, day)">
                            <!-- CAPAS DE EVENTOS -->
                            <div v-for="(event, idx) in getEventsForDay(month.month, month.year, day)"
                                :key="event.titulo + idx" class="event-layer"
                                :style="getEventLayerStyle(event.color, idx)">
                            </div>

                            <!-- NÚMERO DEL DÍA -->
                            <span class="day-number" :style="getDayNumberStyle(month.month, month.year, day)">
                                {{ day }}
                            </span>

                            <!-- TOOLTIP -->
                            <div v-if="getEventsForDay(month.month, month.year, day).length" class="tooltip">
                                <div v-for="event in getEventsForDay(month.month, month.year, day)" :key="event.titulo"
                                    class="tooltip-item"
        ...
        <!-- MODAL -->
        <div v-if="isDayModalOpen" class="day-modal-overlay" @click.self="isDayModalOpen = false">
            <div class="day-modal">
                <h3>Eventos del {{ selectedDayLabel }}</h3>

                <div v-for="event in selectedDayEvents" :key="event.titulo" class="day-modal-event" :style="{
        ...
        <!-- TOAST -->
        <ion-toast :is-open="isToastOpen" :message="toastMessage" :color="toastColor" duration="2000" position="top"
