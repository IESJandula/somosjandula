...
export const obtenerTiposActuador = async (toastMessage, toastColor, isToastOpen) => {
  const tokenPropio = await obtenerTokenJWTValido(toastMessage, toastColor, isToastOpen);

  const response = await fetch(automationsApiUrl + "/automations/admin/tipo/actuador", {
    method: "GET",
    headers: {
      Authorization: `Bearer ${tokenPropio}`,
    },
...
export const obtenerTiposSensores = async (toastMessage, toastColor, isToastOpen) => {
  const tokenPropio = await obtenerTokenJWTValido(toastMessage, toastColor, isToastOpen);

  const response = await fetch(automationsApiUrl + "/automations/admin/tipo/sensor", {
    method: "GET",
    headers: {
      Authorization: `Bearer ${tokenPropio}`,
    },
...
export const actualizarEstadoProyectorYObtenerAccion = async (
  toastMessage,
  toastColor,
  isToastOpen,
  mac,
  ...
  const response = await fetch(
    automationsApiUrl + '/automations/admin/actualizacion/actuador/proyector/estado',
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${tokenPropio}`,
      ...
      body: JSON.stringify({
        mac: mac?.value ?? mac,
        estadoProyector: estadoProyector?.value ?? estadoProyector,
      }),
    }
  );
